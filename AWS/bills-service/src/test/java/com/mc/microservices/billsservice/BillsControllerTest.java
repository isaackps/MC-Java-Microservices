...
class BillsControllerTest {
	...
	public void getUserBillsTest() throws Exception {
		...
		mockMvc.perform(get("/api/getBills/{username}", "isaac"))
		.andExpect(status().isOk());
	...
	public void addBillsTest() throws Exception {
		...
		mockMvc.perform(post("/api/addBill")
	...
	public void payBillsTest() throws Exception {
		...
		PayBillDetails payBillDetails = new PayBillDetails("isaac", 1, 1);
		Bills bill1 = new Bills(1L, "isaac", "credit card bills", 100, "dbs", "unpaid");
		Bills billPaid = new Bills(1L, "isaac", "credit card bills", 100, "dbs", "paid");
		
		
		when(billsService.checkAndPayBill(payBillDetails)).thenReturn(true);
		when(billsRepository.getOne(1L)).thenReturn(bill1);
		when(billsRepository.save(billPaid)).thenReturn(billPaid);
		
		mockMvc.perform(post("/api/payBills")
				.contentType(MediaType.APPLICATION_JSON)
				.content(payload))
				.andExpect(status().isOk());
				//.andExpect(jsonPath("$.username", Matchers.is("isaac")));
